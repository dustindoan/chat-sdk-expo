...
export function createStatefulAgent<TStates extends string>(
  ...
  const resolveModel = options?.resolveModel || ((model: ModelShorthand | ModelConfig | undefined) => getModel(model, options?.apiKey));

  // Default model (fallback to 'haiku' if nothing specified)
  const defaultModel = resolveModel(workflow.defaultModel || options?.defaultModel || 'haiku');
  ...
  const agent = new ToolLoopAgent({
    ...
    prepareStep: ({ stepNumber, steps }) => {
      ...
      return {
        model: stateConfig.model ? resolveModel(stateConfig.model) : undefined,

 ...
 *
 * This package provides framework-agnostic types. React-specific UI definitions
 * (like ArtifactDefinition with React components) should be defined in consuming apps.
 */
 ...
 */
export function isArtifactDataType(part: unknown): part is ArtifactDataType {
...
/**
 * Artifact action context - passed to action handlers
 */
...
/**
 * Artifact content props (framework-agnostic base)
 * Consuming apps can extend this with their own component types
 */
export interface ArtifactContentPropsBase<M = unknown> {
  /** Current content */
  content: string;
  /** Streaming status */
  status: ArtifactStatus;

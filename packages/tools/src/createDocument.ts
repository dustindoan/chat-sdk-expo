 ...
 *
 * This is a factory function that takes database adapters as parameters,
 * allowing apps to provide their own persistence layer.
 */
 ...
 */
export const artifactKinds: [ArtifactKind, ...ArtifactKind[]] = ['text', 'code'];
...
export interface CreateDocumentToolProps {
  ...
  getDocumentHandler: (kind: ArtifactKind) => DocumentHandler | undefined;
  /** Save document to database (optional - if not provided, document won't be persisted) */
  saveDocument?: (doc: DocumentToSave) => Promise<void>;
}

/**
 * Create the createDocument tool
 * Factory function that takes dataStream context and database adapters
 */
...
The document displays in a dedicated panel. After calling, briefly describe what was created.`,
    inputSchema: z.object({
      ...
      kind: z
        .enum(artifactKinds)
        .describe('The type of document: "text" for written content, "code" for programming'),
    }),
    execute: async ({ title, kind }: { title: string; kind: ArtifactKind }) => {
      ...
      const wrappedDataStream: DataStreamWriter = {
        write: (data) => {
          ...
          if (data.type === 'data-id') {
            ...
            dataStream.write({ ...data, data: { docId: id } } as unknown as typeof data);
          } else {
            // For value-data events, use { value, docId } format
            const value = typeof data.data === 'object' && 'value' in data.data
              ? data.data.value
              : data.data;
            dataStream.write({ ...data, data: { value, docId: id } } as unknown as typeof data);
          }
        },
      };
      ...
      // Save document to database if saveDocument is provided
      if (saveDocument) {
        try {
          await saveDocument({
            id,
